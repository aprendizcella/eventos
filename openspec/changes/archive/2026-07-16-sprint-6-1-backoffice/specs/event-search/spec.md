...
The system MUST index only public events with status `published`. Suspended events MUST be excluded. The searchable text MUST cover event title and description only. Structured attributes MAY be indexed for filtering, but MUST NOT become free-text search fields.
...
#### Scenario: Suspended event is excluded
- GIVEN an event with status `suspended`
- WHEN the index is refreshed
- THEN the event MUST NOT be indexed
