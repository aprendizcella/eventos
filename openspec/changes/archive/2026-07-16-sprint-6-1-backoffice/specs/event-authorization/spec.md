...
Define organizer-scoped event permissions and global admin access boundaries.
...
The system MUST allow users with `super_admin` or `platform_admin` role to read and manage any event or venue across all organizers, using the global Spatie team ID `0` context.
...
#### Scenario: Tenant context does not grant access
- GIVEN a tenant-scoped permission exists for a different organizer
- WHEN a global admin request is evaluated
- THEN only team `0` permissions MUST be used
