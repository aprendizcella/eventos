...
The system MUST provide an `EventStatus` enum with cases: `draft`, `published`, `paused`, `cancelled`, `suspended`. The initial status on creation MUST be `draft`.
...
The system MUST enforce valid status transitions via a `PublishEventAction`, `PauseEventAction`, `CancelEventAction`, and suspension/restore actions. Allowed transitions:
- `draft` → `published`
- `published` → `paused`
- `published` → `cancelled`
- `published` → `suspended`
