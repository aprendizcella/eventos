...
@section('content')
    <div class="mx-auto max-w-2xl space-y-6">
        {{-- Header --}}
        <div>
            <h1 class="text-2xl font-semibold text-gray-900 dark:text-gray-100">Edit Organizer</h1>
            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                Update organizer details and settings.
            </p>
        </div>

        {{-- Form --}}
        <form action="{{ route('organizers.update', $organizer) }}" method="POST" class="space-y-6 rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-900">
            @csrf
            @method('PUT')

            <x-form.field name="name" label="Name" :value="old('name', $organizer->name)" required autofocus />

            <x-form.field name="slug" label="Slug" :value="old('slug', $organizer->slug)" required>
                <x-slot:help>Unique identifier for URLs. Use lowercase letters, numbers, and hyphens.</x-slot:help>
            </x-form.field>

            <x-form.field name="domain" label="Domain" :value="old('domain', $organizer->domain)">
            ...
            <div class="mb-4">
                ...
                <select id="status" name="status" class="mt-1 w-full rounded border border-gray-400 bg-white px-3 py-2 focus:border-blue-500 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100">
                    <option value="active" @selected(old('status', $organizer->status) === 'active')>Active</option>
                    <option value="inactive" @selected(old('status', $organizer->status) === 'inactive')>Inactive</option>
                </select>
                @error('status')
            ...
            <div class="flex items-center justify-end gap-3 border-t border-gray-200 pt-6 dark:border-gray-700">
                ...
                <x-ui.button type="submit">Update Organizer</x-ui.button>
            </div>
        </form>
    </div>
@endsection
