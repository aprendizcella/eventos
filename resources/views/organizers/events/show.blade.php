...
@section('content')
    <div class="space-y-6" x-data="{ activeTab: 'overview' }">
        ...
        {{-- Tab Content: Attendees & Check-in --}}
        <div x-show="activeTab === 'attendees'" class="space-y-4" x-cloak x-data="{ subView: 'list' }" @open-qr-scanner.window="subView = 'scanner'" @close-qr-scanner.window="subView = 'list'">
            <div x-show="subView === 'list'">
                <livewire:organizers.events.attendee-list :event="$event" />
            </div>
            <div x-show="subView === 'scanner'" x-cloak>
                <livewire:organizers.events.check-in :event="$event" />
            </div>
        </div>
